const { assert, expect } = require("chai")
const { network, getNamedAccounts, deployments, ethers } = require("hardhat")
const { developmentChains, networkConfig } = require("../../helper-hardhat-config")
const fs = require("fs")

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT Tests", function () {
          let deployer, dynamicSvgNft, mockV3Aggregator
          const chainId = network.config.chainId

          const lowSVGImageuri =
              "data:image/svg+xml;base64,"

          const highSVGImageuri =
              "data:image/svg+xml;base64,"

          const lowTokenUri =
              "data:application/json;base64,"

          const highTokenUri =
              "data:application/json;base64,"

          beforeEach(async function () {
              deployer = (await getNamedAccounts()).deployer
              await deployments.fixture(["mocks", "dynamicSvgNft"])
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator", deployer)
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft", deployer)
          })

          describe("constructor", function () {
              it("sets starting values correctly", async function () {
                  const orignalLowSVG = await fs.readFileSync("./images/dynamicNft/frown.svg", {
                      encoding: "utf8",
                  })
                  const orignalHighSVG = await fs.readFileSync("./images/dynamicNft/happy.svg", {
                      encoding: "utf8",
                  })
                  const generatedImageLowUri = await dynamicSvgNft.svgToImageURI(orignalLowSVG)
                  const generatedImageHighUri = await dynamicSvgNft.svgToImageURI(orignalHighSVG)
                  assert.equal(generatedImageLowUri, lowSVGImageuri)
                  assert.equal(generatedImageHighUri, highSVGImageuri)
                  assert.equal(await dynamicSvgNft.getPriceFeed(), mockV3Aggregator.address)
              })
          })

          describe("mintNft", function () {
              it("emits an event when mint and get low", async function () {
                  const highValue = ethers.utils.parseEther("10000000000000000000")
                  await expect(dynamicSvgNft.mintNft(highValue)).to.emit(
                      dynamicSvgNft,
                      "CreatedNFT"
                  )
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(tokenCounter.toString(), "1")
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  assert.equal(tokenURI, lowTokenUri)
              })

              it("mint and get high", async function () {
                  const highValue = ethers.utils.parseEther("100") // $100,000,000 dollar per ether. Maybe in the distant future this test will fail...
                  const txResponse = await dynamicSvgNft.mintNft(highValue)
                  await txResponse.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(1)
                  assert.equal(tokenURI, highTokenUri)
              })
          })
      })
